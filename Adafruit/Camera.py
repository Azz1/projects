...
# Camera using Picamera2 library
class PiCamera(Camera):
  ...
  def snapshot(self) : 
    ...
    track_err = False
    img = Image.new(mode="RGB", size=(ControlPackage.width, ControlPackage.height))
    try:
      ControlPackage.imageseq = ControlPackage.imageseq + 1
      localtime   = time.localtime()
      fname = 'temp/image-' + str(ControlPackage.imageseq) + '-' + time.strftime("%Y%m%d-%H%M%S", localtime) + '.jpg'
      ...
      combined_config = self.picam2.create_still_configuration({"size": (ControlPackage.width, ControlPackage.height)},
                                                      transform=Transform(
                                                        vflip=(True if ControlPackage.vflip == 'true' else False),
                                                        hflip=(True if ControlPackage.hflip == 'true' else False)
                                                      ))

      self.picam2.configure(combined_config)
      with self.picam2.controls as controls:
        controls.AeEnable = False
        controls.ExposureTime = int(ss)
    ...
    if ControlPackage.ref0_x != ControlPackage.ref1_x or ControlPackage.ref0_y != ControlPackage.ref1_y:
      ...
      try:
        ...
        output = BytesIO()           # Create a BytesIO stream
        img.save(output, format='JPEG')  # Save the image as JPEG to the stream
        imgstr = base64.b64encode(output.getvalue()).decode('utf-8')
...
ControlPackage.camera = PiCamera()
